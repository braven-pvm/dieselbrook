...
namespace Nop.Plugin.Shipping.FixedByWeightByTotal
{
    ...
    public class FixedByWeightByTotalComputationMethod : BasePlugin, IShippingRateComputationMethod
    {
        ...
        //task 616 Custom shipping rule
        private decimal GetCustomRate(CustomShippingByWeightByTotalRecord shippingByWeightByTotalRecord, decimal subTotal, decimal weight)
        {
            //additional fixed cost
            var shippingTotal = shippingByWeightByTotalRecord.AdditionalFixedCost;
        ...
        public async Task<int[]> PrioritizeRolesExcludingRegisteredAsync(int[] customerRoleIds)
        {
            var registeredRole = await _customerService.GetCustomerRoleBySystemNameAsync(NopCustomerDefaults.RegisteredRoleName);

            // Check if the customer has more than one role
            if (customerRoleIds.Length > 1)
            {
                // Check if the RegisteredRole ID exists in customerRoleIds
                if (customerRoleIds.Contains(registeredRole.Id))
                {
                    // Remove the RegisteredRole ID from customerRoleIds
                    customerRoleIds = customerRoleIds.Where(id => id != registeredRole.Id).ToArray();
                }
            }
        ...
        public async Task<GetShippingOptionResponse> GetShippingOptionsAsync(GetShippingOptionRequest getShippingOptionRequest)
        {
            ...
            if (_fixedByWeightByTotalSettings.ShippingByWeightByTotalEnabled)
            {
                ...
                //roles are being prioritized, with the "Registered" role being excluded as "Registered" is common customer role
                //Applied this changes as faced issue while If a customer has both the 'Client' role (under the first shipping rule)
                //and the 'Registered' role (under the second shipping rule) in this situation Registered role will be excluded and Client role will be prioritized
                customerRoleIds = await PrioritizeRolesExcludingRegisteredAsync(customerRoleIds);
                ...
                foreach (var shippingMethod in await _shippingService.GetAllShippingMethodsAsync(countryId))
                {
                    ...
                    //task 616 Custom shipping rule customization

                    //check custom shipping rule enabled or not
                    var isCustomShippingRule = await _customShippingRuleService.IsCustomShippingRuleEnableAsync();
                    if (isCustomShippingRule)
                    {
                        //find custom shipping rule based on customer roles
                        var shippingByWeightByTotalRecord = await _customShippingRuleService.FindRecordsAsync(
                           shippingMethod.Id, storeId, warehouseId, countryId, stateProvinceId, zip, weight, subTotal, customerRoleIds);
                        ...
                        {
                            //get custom shipping rate
                            rate = GetCustomRate(shippingByWeightByTotalRecord, subTotal, weight);
