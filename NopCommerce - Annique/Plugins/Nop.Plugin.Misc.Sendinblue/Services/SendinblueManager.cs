            ...
            try
            {
                ...
                if (customer != null)
                {
                    ...
                    if (countryIsoCode > 0 && !string.IsNullOrEmpty(phone))
                    {
                        ...
                        #region Customization for phone number format with country code

                        // The following line was commented out as it was causing issues when creating contacts via the API.
                        // sms = phone.Replace($"+{phoneCode}", string.Empty);

                        // Instead of removing the country code, the new logic ensures the phone number is correctly formatted
                        // by adding the appropriate country code (if missing), while keeping existing country codes intact.
                        if (!phone.StartsWith($"+{phoneCode}"))
                            sms = $"+{phoneCode}{phone}";  // Prepend the country code to the phone number if it doesn't already start with it.
                        else
                            sms = phone;  // If the phone number already starts with the correct country code, keep it as is.
