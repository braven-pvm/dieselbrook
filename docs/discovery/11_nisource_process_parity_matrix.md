...
| `campaign/campaign.prg`, `campaign/Campaign.html`, `campaign/campaigns.js`, `campaign/Campaign_class.prg` | campaign administration and product-exposure UI module | Web Connection page rendering, campaign callbacks, ERP campaign procedures, `Campaign` / `CampDetail` / `CampSku` family objects | campaign summary UI, SKU-to-campaign editing, publish-to-stage (Namibia copy action present in legacy but out of scope) | `CampaignAdministrationService` plus `CampaignPricingAdminUi` |
